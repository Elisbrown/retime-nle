...
- Final Cut Pro: `No declaration for attribute "src" of element "clip"` — you are on retime-nle 0.1.0, whose FCPXML put `src` on `<clip>`. Upgrade to 0.1.1+, which emits `<resources>` + `<asset-clip ref="…">` per the FCPXML DTD.
